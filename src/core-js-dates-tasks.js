...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return (new Date(dateEnd) - new Date(dateStart)) / (24 * 3600000) + 1;
}
...
function isDateInPeriod(date, period) {
  const dateDate = (d) => new Date(d);
  return (
    dateDate(period.start) <= dateDate(date) &&
    dateDate(date) <= dateDate(period.end)
  );
}
...
function formatDate(dateStr) {
  ...
  const displayHour = hoursUTC % 12 || 12;

  const period = hoursUTC >= 12 ? 'PM' : 'AM';

  const pad = (num) => num.toString().padStart(2, '0');

  return `${month}/${day}/${year}, ${displayHour}:${pad(minutes)}:${pad(seconds)} ${period}`;
}
/**
...
function getNextFridayThe13th(date) {
  const newDate = new Date(date.getFullYear(), date.getMonth(), 13);
  if (newDate > date && newDate.getDay() === 5) return newDate;
  do {
